...
const CustomerMarker: React.FC<{
  ...
  // Google hazır değilse render etme (erken erişim hatalarını engeller)
  ...
  return (
    ...
    >
      {isSelected && (
        <InfoWindow
          // 🔧 Esas düzeltme: InfoWindow’a konum veriyoruz (anchor ya da position şart)
          position={{ lat: customer.lat, lng: customer.lng }}
          // Alternatif: anchor={markerRef.current as unknown as google.maps.MVCObject}
        ...
        >
          <div className="space-y-1">
            ...
            <div>
              Tel:{" "}
              <a className="text-[#0099CB] underline" href={toTelHref(customer.phone)}>
                {customer.phone}
              </a>
            </div>
            <a
...
const RouteMapScreen: React.FC<Props> = ({ customers, salesRep }) => {
  ...
  const optimizeRoute = useCallback(async () => {
    if (!directionsService.current || !(window as any).google?.maps) return;

    setLoading(true);
    const allCustomers = [...baseCustomers];
    const origin = { lat: rep.lat, lng: rep.lng };

    // Yıldızlı müşteri varsa onu listenin başına alıyoruz (UI beklentisi)
    const customersForWaypoints = starredId
      ? [allCustomers.find((c) => c.id === starredId)!, ...allCustomers.filter((c) => c.id !== starredId)]
      : allCustomers;

    const waypointLocations = customersForWaypoints.map((c) => ({
    ...
    try {
      const request: google.maps.DirectionsRequest = {
        ...
        waypoints: waypointLocations.slice(0, waypointLocations.length - 1),
      ...
      if (response && response.routes && response.routes.length > 0) {
        ...
        // waypoint_order, gönderdiğimiz WAYPOINT dizisinin (destination hariç) indeksleridir
        const order = route.waypoint_order || [];
        const reordered = order.map((i) => customersForWaypoints[i]);
        // destination (son durak) ekle
        reordered.push(customersForWaypoints[customersForWaypoints.length - 1]);

        // UI'da yıldızlıyı ilk durak olarak göstermek istersek öne sabitle
        const finalList =
          starredId && customersForWaypoints.length
            ? [customersForWaypoints[0], ...reordered.filter((c) => c.id !== customersForWaypoints[0].id)]
            : reordered;

        setOrderedCustomers(finalList);

        const totalDistance = route.legs?.reduce((acc, leg) => acc + (leg.distance?.value || 0), 0) || 0;
  ...
  // Rep marker ikonu (window.google guard’ı)
  const repIcon =
  ...
  return (
    <div className="relative w-full">
      ...
      >
        ...
        >
          <Marker position={{ lat: rep.lat, lng: rep.lng }} icon={repIcon} />
        ...
        >
          ...
          <div className="bg-white/90 rounded-l-xl shadow-md px-6 py-4 flex flex-col gap-3 min-w-[270px] max-w-[21.6rem] h-full">
            ...
            <div className="text-[11px] text-gray-600">
              ⭐ Bir müşteriyi yıldızlarsan rota önce o müşteriye gider; kalan duraklar en kısa şekilde planlanır. Yıldızı
              değiştirince rota otomatik güncellenir.
            </div>
              ...
              {orderedCustomers.map((c, i) => {
                ...
                return (
                  <div
                    key={c.id}
                    id={`cust-row-${c.id}`}
                    onClick={() => highlightCustomer(c.id)}
