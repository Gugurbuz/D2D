...
const RouteMap: React.FC<Props> = ({ customers, salesRep }) => {
  ...
  const selectCustomer = (id: string, pan = true) => {
    ...
    setSelectedId(id); // state güncelle
    ...
    // yeni popup'ı aç (hemen – state'e bağımlı değil)
  ...
  // Marker/Panel tıklamaları hep selectCustomer kullanır
  useEffect(() => {
    // veri filtresi değiştiğinde eski seçim geçersiz olabilir
          ...
          {orderedCustomers.map((c, i) => (
            <Marker
              ...
              ref={(ref: any) => {
                ...
                else delete markerRefs.current[c.id]; // unmount temizliği
              }}
              eventHandlers={{
              ...
              {orderedCustomers.map((c, i) => {
                ...
                return (
                  <div
                    ...
                    onClick={() => selectCustomer(c.id, true)}
                  >
                    <span
