...
const RouteMapScreen: React.FC<Props> = ({ customers, salesRep }) => {
  ...
  const onMapLoad = useCallback((map: google.maps.Map) => {
    ...
    if ((window as any).google?.maps) {
      ...
      directionsRenderer.current = new window.google.maps.DirectionsRenderer({
        ...
        polylineOptions: { strokeColor: "#0099CB", strokeWeight: 7 },
      });
  ...
  const optimizeRoute = useCallback(async () => {
    ...
    try {
      // Son elemanı "destination", diğerlerini waypoints olarak gönder
      const request: google.maps.DirectionsRequest = {
        origin: { lat: rep.lat, lng: rep.lng },
      ...
      if (response && response.routes && response.routes.length > 0) {
        ...
        // waypoint_order: gönderdiğimiz waypoints (destination hariç) için sıralama
        ...
        // Yıldızlı ilk görünsün (rota polylinesi farklı sıralayabilir; UI ilk durak olarak gösterir)
        const finalList =
  ...
  // Rep ikonunda küçük guard
  const repIcon =
    (window as any).google?.maps
      ? { url: "https://companieslogo.com/img/orig/ENJSA.IS-d388e8cb.png?t=1720244491", scaledSize: new window.google.maps.Size(32, 32) }
      : undefined;
    ...
    <div className="relative w-full">
      ...
      <div ref={mapWrapperRef} className="relative h-[560px] w-full rounded-2xl overflow-hidden shadow-xl">
        <GoogleMap
          ...
          center={{ lat: rep.lat, lng: rep.lng }}  // 🔧 rep konumuna göre merkez
          zoom={13}
          onLoad={onMapLoad}
          onUnmount={onUnmount}
