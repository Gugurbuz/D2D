...
// DEĞİŞİKLİK: MarkerF'i import ediyoruz
...
// DEĞİŞİKLİK: CustomerMarker için prop tiplerini tanımlıyoruz
...
const CustomerMarker: React.FC<CustomerMarkerProps> = React.memo(({ customer, index, selectedId, starredId, onClick, isLoaded }) => {
  const isSelected = selectedId === customer.id;
  const isStarred = starredId === customer.id;
  const [marker, setMarker] = useState<google.maps.Marker | null>(null);

  const icon = useMemo(() => {
    // DEĞİŞİKLİK: isLoaded prop'unu ve window.google'ı kontrol ediyoruz
  ...
  return (
    // DEĞİŞİKLİK: Marker'ı MarkerF ile değiştiriyoruz
    <MarkerF
...
const RouteMapScreen: React.FC<Props> = ({ customers, salesRep }) => {
  ...
  const optimizeRoute = useCallback(async () => {
    if (!directionsService.current || !isLoaded) return;
    setLoading(true);
    ...
    try {
      ...
      if (response.status === "OK" && response.routes.length > 0) {
        ...
        // Önemli: waypoint_order, optimize edilmemiş waypoint listesindeki sıralamayı verir.
        const nonStarredCustomers = allCustomers.filter(c => c.id !== starredId);
        let reorderedCustomers = route.waypoint_order.map(index => nonStarredCustomers[index]);
        
        if (starredId) {
          const starredCustomer = allCustomers.find(c => c.id === starredId)!;
          reorderedCustomers.unshift(starredCustomer);
        }
  ...
  }, [baseCustomers, starredId, rep.lat, rep.lng, isLoaded]);
  ...
  useEffect(() => {
    if (starredId !== null && isLoaded) optimizeRoute();
  }, [starredId, optimizeRoute, isLoaded]);
        ...
        >
          ...
          {orderedCustomers.map((customer, index) => (
            <CustomerMarker
              ...
              isLoaded={isLoaded} // DEĞİŞİKLİK: isLoaded prop'unu iletiyoruz
            />
          ))}
        </GoogleMap>
