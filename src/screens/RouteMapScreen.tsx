  ...
  const handleOptimize = async () => {
    try {
      ...
      if (!starredId) {
        ...
        const tripSeed = [{ lat: star.lat, lng: star.lng, ref: star }, ...others.map(c => ({ lat: c.lat, lng: c.lng, ref: c }))];
      ...
      console.error(e);
      // OSRM down ise fallback: sırayla bağla + Haversine toplamı
      const seq: LatLng[] = (() => {
      ...
      <div className="relative h-[560px] w-full rounded-2xl overflow-hidden shadow-xl">
        ...
        >
          ...
          <Marker
            ...
            icon={repIcon}
            whenCreated={(m) => { /* temsilci işaretçisi referansı gerekmez */ }}
          >
          ...
          {orderedCustomers.map((c, i) => (
            <Marker
              ...
              zIndexOffset={1000 - i}
              whenCreated={(m) => { markerRefs.current[c.id] = m; }}
              eventHandlers={{
                click: () => {
                  setSelectedId(c.id);
                  const m = markerRefs.current[c.id];
        ...
        {/* Sağ panel */}
        <div
          className={`absolute top-4 right-0 bottom-4 z-10 transition-transform duration-300 ${panelOpen ? "translate-x-0" : "translate-x-[calc(100%-1.5rem)]"} flex`}
