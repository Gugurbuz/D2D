...
async function osrmTripSafe(points: { lat: number; lng: number }[]) {
  try {
    ...
    // FIX: destination=any (last yerine) → 400 alma olasılığı azalır
    const url = encodeURI(
    ...
    try {
      ...
      if (!starredId) {
        ...
        if (tripData) {
          ...
          const sortedCustomers = ordered
            .slice(1) // rep’i at
            .map((p: any) => p.ref as Customer);
        ...
        // 2) Fallback: greedy + segment/düz çizim
        ...
        // ⭐ var: rep -> star, sonra star + diğerleri
        ...
        // Trip2 de yoksa: greedy (star sabit ilk) + segment/düz
