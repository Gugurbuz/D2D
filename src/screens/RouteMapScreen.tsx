...
const anadoluCustomers: Customer[] = []; // Burada kendi müşteri verilerini koyuyorsun
...
const RouteMap: React.FC<Props> = ({ customers, salesRep }) => {
  ...
  return (
    <div className="relative w-full h-[560px] rounded-2xl overflow-hidden shadow-xl">
      {/* Sticky bar artık haritanın içinde ve üstte */}
      <div className="absolute top-0 left-0 right-0 z-20 bg-white/80 py-2 px-3 flex items-center justify-between shadow-sm border-b">
        <div className="flex items-center gap-2 text-gray-900 text-xs">
          <RouteIcon className="w-4 h-4 text-[#0099CB]" />
          <span className="font-semibold">Toplam: {fmtKm(routeKm)}</span>
        </div>
