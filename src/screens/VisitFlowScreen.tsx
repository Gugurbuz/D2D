...
// --- GEREKLİ TİPLER ---
// Normalde ayrı bir dosyada olan Customer tipini önizleme için buraya ekledim.
...
const VisitFlowScreen: React.FC<VisitFlowProps> = ({ customer, onCloseToList, onCompleteVisit }) => {
  ...
  const handleFinalizeVisit = () => {
    ...
    if (state.rejectionReason) {
        ...
        const formattedDate = new Date(state.rescheduledDate).toLocaleDateString('tr-TR', {
...
const FinalizeVisitScreen: React.FC<{
  ...
  const reasonsConfig = {
    Rejected: ['Fiyatı pahalı buldu', 'Mevcut sağlayıcısından taahhütü var', 'Hizmet kalitesine güvenmiyor', 'Teklifle ilgilenmiyor'],
    Unreachable: ['Adres hatalı / eksik', 'Adreste bulunamadı', 'Kapıyı açmadı'],
    Evaluating: ['Teklifi inceleyecek', 'Eşiyle/ortağıyla görüşecek', 'Geri dönüş yapacak']
  };
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm p-6 animate-fade-in">
      <div className="text-center">
        ...
        <p className="text-gray-600 mb-6">Lütfen ziyaret detaylarını belirtip notlarınızı ekleyin.</p>
      </div>
      
      {state.visitStatus === 'Postponed' && (
        <div className="text-left my-6">
            <label htmlFor="rescheduleDate" className="block text-sm font-medium text-gray-700 mb-2">Yeni Randevu Tarihi Belirleyin</label>
            <input type="date" id="rescheduleDate" value={state.rescheduledDate || ''} onChange={(e) => dispatch({ type: 'SET_RESCHEDULED_DATE', payload: e.target.value })} className="w-full p-2 border rounded-lg" min={new Date().toISOString().split('T')[0]} />
        </div>
      )}
      
      {reasons && (
         <div className="text-left my-6">
            <label className="block text-sm font-medium text-gray-700 mb-2">Lütfen bir sebep seçin</label>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                {reasons.map(reason => (
                    <label key={reason} className={`p-3 border rounded-lg flex items-center gap-3 cursor-pointer transition-colors ${state.rejectionReason === reason ? 'bg-blue-50 border-blue-400 ring-2 ring-blue-300' : 'hover:bg-gray-50'}`}>
                        <input type="radio" name="rejectionReason" value={reason} checked={state.rejectionReason === reason} onChange={(e) => dispatch({ type: 'SET_REJECTION_REASON', payload: e.target.value })} className="h-4 w-4 text-[#0099CB] border-gray-300 focus:ring-[#0099CB]" />
      ...
      <div className="text-left mt-6">
        <label htmlFor="visitNotes" className="block text-sm font-medium text-gray-700 mb-1">Ek Notlar</label>
        <textarea id="visitNotes" value={state.notes} onChange={(e) => dispatch({ type: 'SET_NOTES', payload: e.target.value })} rows={3} className="w-full p-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-[#0099CB]" placeholder="Eklemek istediğiniz diğer detaylar..." />
      </div>
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm p-6 animate-fade-in">
        <div className="flex items-center justify-between gap-3 mb-4">
            <div className="flex items-center gap-3"><ScanLine className="w-5 h-5 text-[#0099CB]" /><h3 className="text-lg font-semibold">2. Kimlik Doğrulama</h3></div>
            <div className="flex items-center gap-2 p-2 rounded-lg bg-yellow-50 border border-yellow-300"><input type="checkbox" id="bypass-verification" checked={isBypassChecked} onChange={(e) => handleBypassToggle(e.target.checked)} className="h-4 w-4 rounded text-[#0099CB] focus:ring-[#0099CB]" /><label htmlFor="bypass-verification" className="text-sm font-medium text-yellow-800">[TEST] Doğrulamayı Atla</label></div>
        </div>
        <fieldset disabled={isBypassChecked}>
          <div className={`grid md:grid-cols-2 gap-6 items-start transition-opacity ${isBypassChecked ? 'opacity-40' : 'opacity-100'}`}>
              <div>
                  ...
                  <div className="mt-4">
                      {state.ocrStatus === 'idle' && (<button onClick={stream ? handleCaptureAndOcr : startCamera} className="w-full bg-[#0099CB] text-white px-4 py-2 rounded-lg flex items-center justify-center gap-2"><Camera className="w-4 h-4" /> {stream ? 'Fotoğraf Çek ve Doğrula' : 'Kamerayı Başlat'}</button>)}
        ...
        <div className="mt-6 flex justify-between">
            ...
            <button onClick={() => dispatch({ type: 'SET_STEP', payload: 3 })} disabled={!isVerified} className={`px-6 py-3 rounded-lg text-white ${isVerified ? 'bg-[#0099CB]' : 'bg-gray-300'}`}>Devam Et</button>
        </div>
    </div>
  );
};
 ...
 return (
   <div className="bg-white rounded-xl shadow-sm p-6 animate-fade-in">
     ...
     <div className="mt-6 flex justify-between">
       ...
       <button onClick={() => dispatch({ type: "SET_STEP", payload: 4 })} disabled={!canContinue} className={`px-6 py-3 rounded-lg text-white ${canContinue ? "bg-[#0099CB]" : "bg-gray-300"}`}>Sözleşmeyi Onayla ve Bitir</button>
     </div>
...
const CompletionStep: React.FC<{ customer: Customer; dispatch: React.Dispatch<Action>; onComplete: () => void; }> = ({ customer, onComplete }) => (
...
const SignaturePadModal: React.FC<{ onClose: () => void; onSave: (dataUrl: string) => void; }> = ({ onClose, onSave }) => {
  ...
  return (
    <div role="dialog" aria-modal="true" className="fixed inset-0 z-[10050] bg-black/50">
      ...
      <div className="relative h-screen w-screen">
          ...
          <div className="absolute bottom-4 left-1/2 -translate-x-1/2 text-gray-400 text-sm pointer-events-none">Lütfen bu alana imzanızı atın</div>
      </div>
    </div>
  );
};
...
const ContractMockPage: React.FC<{ customer: Customer; signatureDataUrl: string | null; scale: "preview" | "full"; }> = ({ customer, signatureDataUrl, scale }) => {
 ...
 return (
   <div className={`relative h-full w-full ${base.pad} bg-white`}>
     <div className="text-center mb-2"><div className={`font-semibold ${base.title} text-gray-900`}>ELEKTRİK SATIŞ SÖZLEŞMESİ</div><div className={`${base.small} text-gray-500`}>Mock • Tek Sayfa</div></div>
     <div className={`space-y-2 ${base.body} text-gray-800`}><p>İşbu sözleşme; <strong>{customer.name}</strong> ({customer.address}, {customer.district}) ile Enerjisa Satış A.Ş. arasında, elektrik tedariki kapsamındaki hak ve yükümlülükleri belirlemek üzere{" "}{new Date().toLocaleDateString()} tarihinde akdedilmiştir.</p><ol className="list-decimal ml-5 space-y-1"><li>Teslim noktasında ölçüm değerleri esas alınır.</li><li>Faturalandırma aylık dönemler itibarıyla yapılır.</li><li>Ödeme süresi fatura tebliğinden itibaren 10 gündür.</li><li>Cayma süresi imzadan itibaren 14 gündür.</li><li>Kişisel veriler 6698 sayılı KVKK kapsamında işlenir.</li></ol></div>
     <div className="mt-6 pt-4 border-t"><div className="grid grid-cols-2 gap-4"><div className="flex flex-col"><div className="border-2 border-dashed border-gray-300 rounded bg-white flex items-center justify-center" style={{ height: sigH }}>{signatureDataUrl ? (<img src={signatureDataUrl} alt="Müşteri İmzası" style={{ maxHeight: imgMaxH, maxWidth: "90%" }} className="object-contain" />) : (<span className={`${base.small} text-gray-400`}>Müşteri İmzası</span>)}</div><div className={`${base.small} mt-1 text-gray-500 text-center`}>Müşteri İmzası</div></div><div className="flex flex-col"><div className="border-2 border-dashed border-gray-300 rounded bg-white flex items-center justify-center" style={{ height: sigH }}><span className={`${base.small} text-gray-400`}>Tedarikçi İmzası</span></div><div className={`${base.small} mt-1 text-gray-500 text-center`}>Tedarikçi İmzası</div></div></div></div>
   </div>
 );
};
...
const ContractModal: React.FC<{ customer: Customer; signatureDataUrl: string | null; onClose: () => void; }> = ({ customer, signatureDataUrl, onClose }) => {
 useEffect(() => { const scrollY = window.scrollY; const { style } = document.body; const htmlStyle = document.documentElement.style; const prev = { overflow: style.overflow, position: style.position, top: style.top, width: style.width, overscroll: htmlStyle.overscrollBehaviorY as string | undefined, }; style.overflow = "hidden"; style.position = "fixed"; style.top = `-${scrollY}px`; style.width = "100%"; htmlStyle.overscrollBehaviorY = "contain"; return () => { style.overflow = prev.overflow; style.position = prev.position; style.top = prev.top; style.width = prev.width; htmlStyle.overscrollBehaviorY = prev.overscroll || ""; window.scrollTo(0, scrollY); }; }, []);
 return (
...
// Sahte müşteri verileri
const mockCustomers: Customer[] = [
    { id: 1, name: 'Ahmet Yılmaz', address: 'Gül Mahallesi, Lale Sokak No: 12/3', district: 'Çankaya/Ankara', phone: '555 123 4567', status: 'Pending' },
    { id: 2, name: 'Ayşe Kaya', address: 'Menekşe Caddesi, Zambak Apartmanı No: 5', district: 'Kadıköy/İstanbul', phone: '555 987 6543', status: 'Pending' },
];

// Bu, VisitFlowScreen bileşenini çalıştıran ana uygulama bileşenidir.
const App = () => {
    ...
    const handleComplete = (customer: Customer, status: VisitStatus, notes: string) => {
        ...
        // Gerçek uygulamada burada veri tabanına kayıt yapılır.
    };
    ...
    return (
        <div className="p-6 max-w-4xl mx-auto bg-gray-50 min-h-screen">
            <h1 className="text-3xl font-bold text-gray-900 mb-6">Müşteri Ziyaret Listesi</h1>
                ...
                {mockCustomers.map(customer => (
                    <div key={customer.id} className="bg-white p-4 rounded-xl shadow-sm flex items-center justify-between">
                        ...
                        <button 
                            ...
                            className="bg-[#0099CB] text-white px-5 py-2 rounded-lg font-semibold hover:bg-[#007ca8] transition-colors"
                        >
                            Ziyareti Başlat
                        </button>
                    </div>
