...
const CheckInScreen: React.FC<{ customer: Customer, onContinue: () => void }> = ({ customer, onContinue }) => {
    ...
    useEffect(() => { setLocationStatus('getting'); setError(null); navigator.geolocation.getCurrentPosition( (position) => { const userLat = position.coords.latitude; const userLng = position.coords.longitude; const dist = getDistance(userLat, userLng, customer.lat, customer.lng); setDistance(dist); setLocationStatus('success'); }, (err) => { setError(`Konum alınamadı: ${err.message}`); setLocationStatus('error'); }, { enableHighAccuracy: true, timeout: 10000, maximumAge: 0 } ); }, [customer.lat, customer.lng]);
    const renderStatus = () => {
        switch (locationStatus) {
            ...
            case 'success':
                ...
                return isClose ? ( <><CheckCircle className="w-4 h-4 text-green-500" /> Müşteri konumundasınız ({distance.toFixed(0)}m).</> ) : ( <><XCircle className="w-4 h-4 text-yellow-500" /> Müşteri konumundan uzaktasınız (~{ (distance / 1000).toFixed(1)} km).</> );
            default: return null;
        }
    };
    return (
        <div className="bg-white rounded-xl shadow-sm p-6 animate-fade-in">
            <div className="flex items-center gap-3 mb-4"><MapPin className="w-6 h-6 text-[#0099CB]" /><h3 className="text-xl font-semibold">Ziyaret Başlangıç Onayı</h3></div>
            ...
            <div className="mt-6 text-right"><button onClick={onContinue} className="bg-[#0099CB] text-white px-6 py-3 rounded-lg inline-flex items-center gap-2 hover:bg-[#007ca8] transition-colors">Check-in Yap ve Devam Et <ChevronRight className="w-4 h-4" /></button></div>
        </div>
    );
};
...
const LeadGenerationModal: React.FC<{ customer: Customer; onClose: () => void }> = ({ customer, onClose }) => {
    ...
    const handleSolutionToggle = (solution: string) => { setSelectedSolutions(prev => prev.includes(solution) ? prev.filter(s => s !== solution) : [...prev, solution] ); };
    const handleSubmit = (e: React.FormEvent) => { e.preventDefault(); console.log("LEAD OLUŞTURULDU:", { customerId: customer.id, solutions: selectedSolutions, kvkk: kvkkAccepted }); setIsSubmitted(true); setTimeout(onClose, 3000); };
    return (
    ...
    return (
        <div className={`relative h-full w-full ${base.pad} bg-white`}>
            ...
            <div className={`space-y-2 ${base.body} text-gray-800`}>
                <p>İşbu sözleşme; <strong>{customer.name}</strong> ({customer.address}, {customer.district}) ile Enerjisa Satış A.Ş. arasında, elektrik tedariki kapsamındaki hak ve yükümlülükleri belirlemek üzere {new Date().toLocaleDateString()} tarihinde akdedilmiştir.</p>
                <ol className="list-decimal ml-5 space-y-1"><li>Teslim noktasında ölçüm değerleri esas alınır.</li><li>Faturalandırma aylık dönemler itibarıyla yapılır.</li><li>Ödeme süresi fatura tebliğinden itibaren 10 gündür.</li></ol>
            </div>
            <div className="mt-6 pt-4 border-t">
                <div className="grid grid-cols-2 gap-4">
                    <div className="flex flex-col"><div className="border-2 border-dashed border-gray-300 rounded bg-white flex items-center justify-center" style={{ height: sigH }}>{signatureDataUrl ? (<img src={signatureDataUrl} alt="Müşteri İmzası" style={{ maxHeight: imgMaxH, maxWidth: "90%" }} className="object-contain" />) : (<span className={`${base.small} text-gray-400`}>Müşteri İmzası</span>)}</div><div className={`${base.small} mt-1 text-gray-500 text-center`}>Müşteri İmzası</div></div>
                    <div className="flex flex-col"><div className="border-2 border-dashed border-gray-300 rounded bg-white flex items-center justify-center" style={{ height: sigH }}><span className={`${base.small} text-gray-400`}>Tedarikçi İmzası</span></div><div className={`${base.small} mt-1 text-gray-500 text-center`}>Tedarikçi İmzası</div></div>
                </div>
            </div>
        </div>
    );
