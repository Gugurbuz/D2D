...
// Voronoi için harita sınırlarını hesaplayan yardımcı fonksiyon
function computeClipRect(reps: Rep[], customers: Customer[], marginKm = 10): [number, number, number, number] {
    ...
    if (!lats.length || !lngs.length) return [28.8, 40.8, 29.4, 41.3];
...
const AssignmentMapScreen: React.FC<Props> = ({ customers, assignments, setAssignments, allReps: reps, onBack }) => {
    ...
    const DrawControl = () => {
        ...
        useEffect(() => {
            ...
            const drawControl = new (L as any).Control.Draw({
                draw: { rectangle: false, circle: false, circlemarker: false, marker: false, polyline: false, polygon: { allowIntersection: false, showArea: false, drawError: { color: "#ff0000", message: "Hatalı çizim" } }, },
            ...
            const onDeleted = () => handleClearSelection();
            map.on((L as any).Draw.Event.CREATED, onCreated);
            map.on((L as any).Draw.Event.DELETED, onDeleted);
            return () => { map.removeControl(drawControl); if (map.hasLayer(drawnItems)) map.removeLayer(drawnItems); drawnItemsRef.current = null; map.off((L as any).Draw.Event.CREATED, onCreated); map.off((L as any).Draw.Event.DELETED, onDeleted); };
        }, [map, handleClearSelection]);
        return null;
    };
    ...
    const handleOptimize = async () => {
        ...
        customers.forEach((c, i) => { const repId = reps[assign[i]]?.id || reps[0].id; nextAssignments[c.id] = repId; });
        setAssignments(nextAssignments);

        const finalCenters: Record<string, LatLng> = {};
        reps.forEach((rep, index) => { finalCenters[rep.id] = centroids[index]; });
    ...
    const voronoiPolys = useMemo(() => {
        ...
        const points: [number, number][] = centers.map(([, pos]) => [pos[1], pos[0]]);
        ...
        centers.forEach(([repId], i) => {
            ...
            if (poly) { polys[repId] = poly.map(([lng, lat]) => [lat, lng]); }
        });
        return polys;
        ...
        <div className="p-4">
            <div className="mb-3 flex items-center justify-between">
                ...
                <div className="flex items-center gap-3">
                    <button onClick={handleOptimize} className="px-4 py-2 rounded-lg bg-[#0099CB] text-white font-semibold hover:opacity-90" title="Müşterileri ekip arasında en dengeli şekilde dağıt">
                        Optimize Et
                    </button>
                </div>
            </div>
                ...
                <MapContainer center={mapCenter as LatLngExpression} zoom={13} style={{ height: "100%", width: "100%" }} className="z-0">
                    ...
                    {reps.map((r) => {
                        ...
                        return poly && poly.length > 0 ? (
                            <Polygon key={`poly-${r.id}`} positions={poly as LatLngExpression[]} pathOptions={{ color: REP_COLORS[r.id] || "#444", fillColor: REP_FILLS[r.id] || "rgba(0,0,0,.1)", fillOpacity: 0.7, weight: 2.5 }}/>
                        ) : null;
                    })}
