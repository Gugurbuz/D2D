            ...
            <Marker key={c.id} position={[c.lat, c.lng]} icon={customerIcons[c.id]}>
                <Popup>
                    <div className="space-y-1">
                        ...
                        <div className="text-xs text-gray-500">Atanan: <b>{currentAssignments[c.id] ? (reps.find((r: Rep) => r.id === currentAssignments[c.id])?.name || currentAssignments[c.id]) : "—"}</b></div>
        ...
        try {
            ...
            for (let i = 0; i < OPTIMIZATION_ITERATIONS; i++) {
                ...
                customerOrder.forEach(custIdx => {
                    ...
                    const costs = centroids.map((centroid, repIdx) => ({ repIdx, cost: haversineKm(point, centroid) })).sort((a, b) => a.cost - b.cost);
    ...
    <div>
        ...
        Tüm müşterileri otomatik dağıtmak için <b>Optimize Et</b> butonuna basın.
            ...
            <div className="relative h-[620px] w-full rounded-2xl overflow-hidden shadow">
                ...
                {!panelOpen && (
                    <button onClick={() => setPanelOpen(true)} className="absolute top-4 right-4 z-[1000] bg-navy shadow-lg p-3 rounded-full hover:bg-gray-100" title="Paneli Aç">
                ...
                {loading && (
                    <>
                        <div className="absolute inset-0 bg-white/60 backdrop-blur-sm flex items-center justify-center z-[1200]">
                            <div className="w-16 h-16 border-4 border-t-transparent border-[#0099CB] rounded-full animate-spin"></div>
                        </div>
                        <div className="absolute top-4 left-1/2 -translate-x-1/2 z-[1300] animate-fade-in-down">
                            <div className="flex items-center gap-3 rounded-xl shadow-2xl px-4 py-3 bg-green-600 text-white">
                                <RouteIcon className="w-5 h-5" />
                                <span className="font-semibold text-sm">Optimizasyon ve Rota Hesaplamaları Yapılıyor...</span>
                            </div>
                        </div>
                    </>
                )}
