...
function customerIcon(color: string, highlighted = false) { const ring = highlighted ? "box-shadow:0 0 0 6px rgba(0,0,0,.08);" : ""; return L.divIcon({ className: "cust-marker dark:rep-marker-dark", html: `<div style="width:22px;height:22px;border-radius:50%;background:${color};border:2px solid #fff;box-shadow:0 2px 6px rgba(0,0,0,.25);${ring}"></div>`, iconSize: [22, 22], iconAnchor: [11, 22], popupAnchor: [0, -18], }); }
...
const DrawControl = ({ onPolygonCreated, onDeleted }: { onPolygonCreated: (layer: L.Polygon) => void, onDeleted: () => void }) => { /* ... Unchanged ... */ return null; };

const AssignmentMapScreen: React.FC<Props> = ({ customers, assignments, setAssignments, allReps: reps, onBack }) => {
    // ... State and memo hooks (unchanged) ...
    ...
    const handleOptimize = async () => { /* ... Unchanged ... */ };
        ...
        <div className="p-4">
            <div className="mb-3 flex items-center justify-between">
                <div className="flex items-center gap-2 text-gray-900 dark:text-gray-100 font-semibold">
                    <button onClick={onBack} className="px-3 py-2 rounded-lg border bg-white dark:bg-gray-700 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-600 inline-flex items-center gap-2">
                ...
                <div className="flex items-center gap-3">
                    <button onClick={handleOptimize} disabled={loading} className="px-4 py-2 rounded-lg bg-[#0099CB] text-white font-semibold hover:opacity-90 disabled:bg-gray-500 disabled:cursor-not-allowed">
            ...
            <div className="relative h-[620px] w-full rounded-2xl overflow-hidden shadow">
                <MapContainer center={mapCenter as LatLngExpression} zoom={13} style={{ height: "100%", width: "100%" }} className="z-0 dark-map-container">
                    {/* TileLayer'ın karanlık mod için özel bir filtresi olabilir, bunu CSS'te yapacağız */}
                ...
                {pendingOptimization && (
                    <div className="absolute top-4 left-1/2 -translate-x-1/2 z-[1000] w-full max-w-2xl px-4">
                        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl overflow-hidden animate-fade-in-down transition-all duration-300 border dark:border-gray-700">
                            <div onClick={() => setIsSummaryExpanded(!isSummaryExpanded)} className="p-4 flex justify-between items-center cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700/50">
                                <div className="flex items-center gap-3">
                                    <div className="bg-blue-100 text-blue-600 p-2 rounded-full"> <Info className="w-6 h-6" /> </div>
                                    <div>
                                        <h3 className="font-bold text-gray-800 dark:text-gray-100">Optimizasyon Ön İzlemesi</h3>
                                ...
                                <ChevronDown className={`w-6 h-6 text-gray-500 dark:text-gray-400 transition-transform duration-300 ${isSummaryExpanded ? 'rotate-180' : ''}`} />
                            </div>
                            {isSummaryExpanded && (
                                <div className="p-4 border-t bg-gray-50/50 dark:bg-black/20 dark:border-gray-700">
                                     <table className="w-full text-left">
                                        <thead><tr className="border-b dark:border-gray-600"><th className="pb-2 font-semibold text-gray-700 dark:text-gray-300">Satış Uzmanı</th><th className="pb-2 font-semibold text-gray-700 dark:text-gray-300 text-center">Müşteri</th><th className="pb-2 font-semibold text-gray-700 dark:text-gray-300 text-right">Rota (km)</th></tr></thead>
                                        <tbody>
                                            {pendingOptimization.summary.map(item => (
                                                <tr key={item.repId} className="border-b dark:border-gray-700 last:border-none">
                                                    <td className="py-2.5 font-medium flex items-center gap-2 text-gray-800 dark:text-gray-200"> 
                            ...
                            <div className="p-3 bg-gray-100 dark:bg-gray-700/50 border-t dark:border-gray-700 flex justify-end gap-3">
                                <button onClick={handleCancelOptimization} className="px-5 py-2 rounded-lg bg-gray-200 dark:bg-gray-600 text-gray-800 dark:text-gray-100 font-semibold hover:bg-gray-300 dark:hover:bg-gray-500 transition-colors"> Çık </button>
                ...
                <div className={`absolute top-0 right-0 bottom-0 z-[1000] transition-transform duration-300 ${panelOpen ? "translate-x-0" : "translate-x-full"} flex`}>
                    <div className="bg-white/95 dark:bg-gray-800/95 backdrop-blur-sm shadow-lg flex flex-col w-[340px] max-w-[90vw] border-l border-gray-200 dark:border-gray-700">
                        <div className="p-4 border-b dark:border-gray-700">
                            <div className="flex items-center justify-between">
                                <h3 className="font-semibold text-gray-800 dark:text-gray-100">Manuel Atama Paneli</h3>
                                <button onClick={() => setPanelOpen(false)} className="p-1 text-gray-500 dark:text-gray-400 hover:text-gray-800 dark:hover:text-gray-100" title="Paneli Kapat"><XCircle className="w-5 h-5"/></button>
                            </div>
                            <p className="text-xs text-gray-600 dark:text-gray-400 mt-1">Haritadan seçim yapıp seçili satış uzmanına atayın.</p>
                        </div>
                        <div className="p-4 space-y-3">
                             <div className="text-sm">
                                <label className="font-medium text-gray-700 dark:text-gray-300">Atanacak Satış Uzmanı:</label>
                                <select value={selectedRepId} onChange={(e) => setSelectedRepId(e.target.value)} className="w-full mt-1 border border-gray-300 dark:border-gray-600 rounded-lg px-2 py-1.5 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100">
                                    {reps.map((r) => (<option key={r.id} value={r.id}>{r.name}</option>))}
                                </select>
                            </div>
                            <div className="text-sm space-y-1 bg-gray-50 dark:bg-gray-700/50 p-3 rounded-md">
                                <div>Seçilen Müşteri: <b>{selectedIds.length}</b></div>
                            </div>
                        </div>
                        <div className="flex-grow overflow-auto p-4 pt-0 space-y-2">
                            {selectedCustomers.length === 0 ? (
                                <div className="text-center text-sm text-gray-500 dark:text-gray-400 py-8">Seçim yapmak için haritadan çokgen çizin veya müşterilere tıklayın.</div>
                            ) : (
                                selectedCustomers.map((c) => (
                                    <div key={c.id} className="p-2 rounded border bg-white dark:bg-gray-700 border-gray-200 dark:border-gray-600 flex items-center justify-between">
                                        <div>
                                            <div className="font-medium text-gray-900 dark:text-gray-100 text-sm">{c.name}</div>
                                            <div className="text-xs text-gray-600 dark:text-gray-400 max-w-[200px] truncate">{c.address}, {c.district}</div>
                                        </div>
                                        <button onClick={() => handleToggleSelection(c.id)} className="p-1 rounded hover:bg-gray-100 dark:hover:bg-gray-600 text-gray-400 hover:text-gray-600" title="Seçimden çıkar"> <XCircle className="w-4 h-4" /> </button>
                                    </div>
                                ))
                            )}
                        </div>
                        {selectedIds.length > 0 && (
                            <div className="p-4 border-t bg-white dark:bg-gray-800 dark:border-gray-700 space-y-2">
                                ...
                                <button onClick={handleClearSelection} className="w-full px-4 py-2 rounded-lg bg-gray-200 dark:bg-gray-600 text-gray-800 dark:text-gray-100 font-semibold inline-flex items-center justify-center gap-2 hover:bg-gray-300 dark:hover:bg-gray-500 transition-colors"> <Trash2 className="w-4 h-4" /> Seçimi Temizle </button>
                            </div>
                        )}
                    </div>
                </div>

                {!panelOpen && ( <button onClick={() => setPanelOpen(true)} className="absolute top-4 right-4 z-[1000] bg-white dark:bg-gray-700 shadow-lg p-3 rounded-full hover:bg-gray-100 dark:hover:bg-gray-600" title="Paneli Aç"> <Users className="w-5 h-5 text-gray-700 dark:text-gray-200" /> </button> )}
                {loading && ( <div className="absolute inset-0 bg-white/60 dark:bg-black/60 backdrop-blur-sm flex items-center justify-center z-[1200]"> <div className="rounded-lg bg-white dark:bg-gray-800 shadow-lg px-6 py-4 text-base font-semibold text-gray-700 dark:text-gray-200"> Optimizasyon ve Rota Hesaplamaları Yapılıyor... </div> </div> )}
