...
const AssignmentMapScreen: React.FC<Props> = ({ customers, assignments, setAssignments, allReps: reps, onBack }) => {
    ...
    // YENİLİK: Optimize edilmiş rotaları ve özet verisini tutacak state'ler
    ...
    const handleAssignSelected = () => { /* ... mevcut kod ... */ };
    const handleToggleSelection = (customerId: string) => { /* ... mevcut kod ... */ };
    const computeRouteForSelection = async (list: Customer[]) => { /* ... mevcut kod ... */ };
    ...
    // YENİLİK: Optimize Et fonksiyonu tamamen yenilendi
    const handleOptimize = async () => {
        ...
        try {
            // 1. Atamaları hesapla (Mevcut K-Means mantığı)
            ...
            for (let t = 0; t < 8; t++) {
                ...
                pts.forEach((p, i) => {
                    ...
                    for (const cand of order) { if (buckets[cand.k].length < target) { chosen = cand.k; break; } }
            ...
            customers.forEach((c, i) => {
                ...
                if (customersPerRep[repId]) {
                    customersPerRep[repId].push(c);
                }
            });

            // 2. Her bir rep için rota hesaplama promise'leri oluştur
            const routePromises = reps.map(async (rep) => {
                ...
                try {
                    ...
                    const routeCoords = tripData.trips[0].geometry.coordinates.map(([lng, lat]: number[]) => [lat, lng] as LatLng);
                    const distanceKm = tripData.trips[0].distance / 1000;
                    return { repId: rep.id, repName: rep.name, customerCount: repCustomers.length, totalKm: distanceKm, coords: routeCoords };
                } catch (error) {
            ...
            // 3. Tüm rotaları paralel olarak hesapla
            ...
            // Bölge çizimlerini de yapalım (Convex Hull)
            ...
            setIsSummaryModalOpen(true); // Özet penceresini aç
    ...
    // YENİLİK: Özet penceresini kapatma fonksiyonu
    const handleCloseSummary = () => {
        setIsSummaryModalOpen(false);
        setOptimizedRoutes({}); // Haritadaki rotaları temizle
        setRegions({}); // Haritadaki bölgeleri temizle
        setSummaryData(null);
    };
        ...
        <div className="p-4">
            ...
            <div className="relative h-[620px] w-full rounded-2xl overflow-hidden shadow">
                <MapContainer center={mapCenter as LatLngExpression} zoom={13} style={{ height: "100%", width: "100%" }} className="z-0">
                    ...
                    {/* YENİLİK: Optimize edilmiş rotaların haritaya çizilmesi */}
                    {Object.entries(optimizedRoutes).map(([repId, route]) => (
            ...
            {/* YENİLİK: Optimizasyon Özeti Penceresi (Modal) */}
                ...
                <div className="absolute inset-0 z-[1100] bg-black/50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg animate-fade-in-up">
                        <div className="p-6 border-b">
                            <h2 className="text-xl font-bold text-gray-800">Optimizasyon Özeti</h2>
                            <p className="text-sm text-gray-600">Müşteriler, satış uzmanlarına aşağıdaki gibi atanmıştır.</p>
                        </div>
                        <div className="p-6">
                            <table className="w-full text-left">
                                <thead>
                                    <tr className="border-b">
                                        ...
                                        <th className="pb-2 font-semibold text-gray-700 text-center">Atanan Müşteri</th>
                                        <th className="pb-2 font-semibold text-gray-700 text-right">Tahmini Rota</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {summaryData.map(item => (
                                        <tr key={item.repId} className="border-b last:border-none">
                                            ...
                                            <td className="py-3 text-right font-mono">
                                                {item.totalKm === null 
                                                    ? <span className="text-red-500">Hesaplanamadı</span> 
                                                    : fmtKm(item.totalKm)
                                                }
                                            </td>
                                        </tr>
                                ...
                                <tfoot className="border-t-2">
                                    <tr className="font-bold">
                                        ...
                                        <td className="pt-3 text-center">{summaryData.reduce((acc, item) => acc + item.customerCount, 0)}</td>
                                        <td className="pt-3 text-right font-mono">{fmtKm(summaryData.reduce((acc, item) => acc + (item.totalKm || 0), 0))}</td>
                                    </tr>
                                </tfoot>
                            </table>
                        </div>
                        <div className="p-4 bg-gray-50 rounded-b-xl flex justify-end">
                            <button onClick={handleCloseSummary} className="px-5 py-2 rounded-lg bg-gray-800 text-white font-semibold hover:bg-gray-900">
                                Tamam
                            </button>
                        </div>
                    </div>
