 ...
 const AssignmentMapScreen: React.FC<Props> = ({ customers, assignments, setAssignments, allReps: reps, onBack }) => { 
 	...
 	const handleOptimize = useCallback(async () => { 
 		...
 		try { 
 			...
 			let assignmentsArr: number[] = new Array(customers.length).fill(-1); 
 			...
 			for (let i = 0; i < OPTIMIZATION_ITERATIONS; i++) { 
 				...
 				customerOrder.forEach(custIdx => { 
 					...
 					assignmentsArr[custIdx] = chosenRep; 
 				}); 
 			...
 			customers.forEach((c, i) => { const repId = reps[assignmentsArr[i]]?.id || reps[0].id; nextAssignments[c.id] = repId; if (customersPerRep[repId]) customersPerRep[repId].push(c); }); 
 	...
 	return ( 
 		<div className="p-4"> 
 			...
 			<div className="relative h-[620px] w-full rounded-2xl overflow-hidden shadow"> 
 				...
 				{loading && (
 					<>
 						{/* Haritanın ortasında dönen ikon */}
 						...
 						{/* Üstte çıkan yeşil bildirim mesajı */}
						{/* DEĞİŞİKLİK BURADA: 'fixed' yerine 'absolute' kullanıldı ve z-index ayarlandı. */}
