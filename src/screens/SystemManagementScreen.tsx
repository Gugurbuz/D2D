...
import {
  Settings2, Wrench, Users, UserPlus, KeyRound, Save, Trash2, LockReset,
  AlertTriangle, Activity, ToggleLeft, ToggleRight, Search, LogOut, RefreshCw,
  Shield, ShieldCheck, Eye, EyeOff, Sparkles, Database
} from "lucide-react";

/* ==== Tipler ==== */
type Role = "admin" | "manager" | "rep" | "viewer";
type AppUser = { id: string; name: string; email: string; role: Role; active: boolean; lastLogin?: string; };
...
const initialFlags: FeatureFlag[] = [
  ...
  { key: "satis_gpt", label: "SatışGPT Asistan", enabled: true, description: "C4C/CRM entegrasyonlu" },
];
...
const Section: React.FC<{title:string; icon?:React.ReactNode; desc?:string;}> = ({ title, icon, desc, children }) => (
  <div className="bg-white rounded-2xl shadow p-5 border border-gray-100">
    <div className="flex items-center gap-2 mb-2">{icon}<h2 className="text-lg font-semibold">{title}</h2></div>
...
const SystemManagementScreen: React.FC = () => {
  ...
  const filteredUsers = useMemo(
    () => users.filter(u => [u.name, u.email, u.role].join(" ").toLowerCase().includes(search.toLowerCase())),
    [users, search]
  );
  ...
  const saveMaintenanceMsg = () => setAudit(a => [{ id: crypto.randomUUID(), ts: new Date().toISOString(), actor: "Admin", action: "BAKIM_MESAJ_GÜNCEL", meta: { maintenanceMsg } }, ...a]);
  const toggleFlag = (key: string) => {
    setFlags(prev => prev.map(f => f.key === key ? { ...f, enabled: !f.enabled } : f));
    setAudit(a => [{ id: crypto.randomUUID(), ts: new Date().toISOString(), actor: "Admin", action: "FLAG_TOGGLE", target: key }, ...a]);
  };
  const runHealthChecks = () => setHealth(h => h.map(x => ({ ...x, status: x.status })));
  const rotateApiKey = () => { const k = "sk-live-" + Math.random().toString(36).slice(2,10) + "-REDACTED"; setApiKey(k); setAudit(a => [{ id: crypto.randomUUID(), ts: new Date().toISOString(), actor: "Admin", action: "API_KEY_ROTATE" }, ...a]); };
  const killAllSessions = () => { setAudit(a => [{ id: crypto.randomUUID(), ts: new Date().toISOString(), actor: "Admin", action: "SESSIONS_KILL_ALL" }, ...a]); alert("Tüm oturumlar sonlandırıldı (mock)."); };
  const clearCaches = () => { setAudit(a => [{ id: crypto.randomUUID(), ts: new Date().toISOString(), actor: "Admin", action: "CACHE_CLEAR" }, ...a]); alert("Önbellek temizlendi (mock)."); };

  const addUser = () => {
    if (!newUser.name?.trim() || !newUser.email?.trim()) return;
    const u: AppUser = { id: crypto.randomUUID(), name: newUser.name!, email: newUser.email!, role: (newUser.role as Role) ?? "viewer", active: newUser.active ?? true };
  ...
  const TabBtn: React.FC<{id: typeof activeTab; label: string; icon: React.ReactNode}> = ({ id, label, icon }) => (
    <button onClick={() => setActiveTab(id)}
      className={`flex items-center gap-2 px-4 py-2 rounded-xl border transition ${activeTab===id?"bg-blue-600 text-white border-blue-600":"bg-white text-gray-700 border-gray-200 hover:bg-gray-50"}`}>
  ...
  return (
    <div className="p-6">
      <div className="flex items-start justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2"><Settings2 className="w-6 h-6" /> Yönetim</h1>
          <p className="text-gray-500">Bakım modu, kullanıcı/rol, güvenlik, bayraklar ve loglar.</p>
        </div>
      ...
      <div className="grid grid-cols-1 gap-6">
            ...
            <Section title="Bakım Modu" icon={<Wrench className="w-5 h-5 text-blue-600" />} desc="Sistemi geçici olarak bakım moduna al.">
              <div className="flex items-center justify-between">
                ...
                <button onClick={toggleMaintenance}
                  className={`px-4 py-2 rounded-lg font-medium border transition ${maintenanceOn?"bg-rose-600 text-white border-rose-600 hover:bg-rose-700":"bg-emerald-600 text-white border-emerald-600 hover:bg-emerald-700"}`}>
              ...
              <textarea value={maintenanceMsg} onChange={(e)=>setMaintenanceMsg(e.target.value)}
                className="w-full rounded-lg border-gray-300 focus:ring-blue-500 focus:border-blue-500" rows={3}/>
              ...
              <div className="grid md:grid-cols-2 gap-3">
                {flags.map(f => (
                  <div key={f.key} className="flex items-center justify-between p-3 rounded-xl border">
                    ...
                    <button onClick={()=>toggleFlag(f.key)} className={`px-3 py-1.5 rounded-lg text-sm border ${f.enabled?"bg-emerald-600 text-white border-emerald-600":"bg-white text-gray-700 border-gray-200 hover:bg-gray-50"}`}>
        ...
        {activeTab==="kullanicilar" && (
          <>
            <Section title="Kullanıcı Ekle" icon={<UserPlus className="w-5 h-5 text-blue-600" />} desc="Yeni kullanıcı oluştur ve rol ata.">
              <div className="grid md:grid-cols-5 gap-3">
                <input placeholder="Ad Soyad" className="md:col-span-1 rounded-lg border-gray-300 focus:ring-blue-500 focus:border-blue-500"
                  value={newUser.name ?? ""} onChange={(e)=>setNewUser(s=>({ ...s, name: e.target.value }))}/>
                <input placeholder="E-posta" className="md:col-span-2 rounded-lg border-gray-300 focus:ring-blue-500 focus:border-blue-500"
                  value={newUser.email ?? ""} onChange={(e)=>setNewUser(s=>({ ...s, email: e.target.value }))}/>
                <select className="rounded-lg border-gray-300 focus:ring-blue-500 focus:border-blue-500"
                  value={(newUser.role as Role) ?? "viewer"} onChange={(e)=>setNewUser(s=>({ ...s, role: e.target.value as Role }))}>
            ...
            <Section title="Kullanıcılar" icon={<Users className="w-5 h-5 text-blue-600" />} desc="Ara, düzenle, şifre sıfırla.">
              <div className="flex items-center gap-2 mb-3">
                <div className="relative flex-1">
                  <Search className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" />
                  <input placeholder="İsim, e-posta veya rol ara…" className="w-full pl-9 rounded-lg border-gray-300 focus:ring-blue-500 focus:border-blue-500"
                ...
                <table className="min-w-full text-sm">
                  <thead>
                    <tr className="text-left text-gray-500">
                      <th className="py-2">Kullanıcı</th><th className="py-2">E-posta</th><th className="py-2">Rol</th>
                      <th className="py-2">Durum</th><th className="py-2">Son Giriş</th><th className="py-2 text-right">İşlemler</th>
                    </tr>
                  </thead>
                  <tbody>
                    {filteredUsers.map(u => (
                      <tr key={u.id} className="border-t">
                        ...
                        <td className="py-2">
                          <select value={u.role} onChange={(e)=>{
                            ...
                            <option value="rep">rep</option><option value="viewer">viewer</option>
                          </select>
                        </td>
                        <td className="py-2">{u.active ? <Badge tone="green">Aktif</Badge> : <Badge tone="gray">Pasif</Badge>}</td>
                        <td className="py-2 text-gray-500">{u.lastLogin ? new Date(u.lastLogin).toLocaleString() : "-"}</td>
                        <td className="py-2">
                          <div className="flex items-center gap-2 justify-end">
                            <button onClick={()=>setEditingUser(u)} className="px-2 py-1 rounded-lg border hover:bg-gray-50" title="Düzenle"><Settings2 className="w-4 h-4" /></button>
                            <button onClick={()=>resetPassword(u.id)} className="px-2 py-1 rounded-lg border hover:bg-gray-50" title="Şifre Sıfırla"><LockReset className="w-4 h-4" /></button>
                            <button onClick={()=>deleteUser(u.id)} className="px-2 py-1 rounded-lg border border-rose-200 text-rose-600 hover:bg-rose-50" title="Sil"><Trash2 className="w-4 h-4" /></button>
                          </div>
                        </td>
                      </tr>
                    ))}
                    {filteredUsers.length===0 && (<tr><td colSpan={6} className="py-6 text-center text-gray-500">Sonuç yok.</td></tr>)}
                  </tbody>
                </table>
              </div>
                ...
                <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
                  <div className="bg-white w-full max-w-lg rounded-2xl p-5 shadow-xl">
                    ...
                    <div className="mt-5 flex items-center justify-end gap-2">
                      ...
                      <button className="px-3 py-2 rounded-lg bg-blue-600 text-white hover:bg-blue-700" onClick={updateUser}><Save className="w-4 h-4 inline-block mr-1" /> Kaydet</button>
                    </div>
                  </div>
                </div>
              )}
            ...
            <Section title="Parola Politikası" icon={<KeyRound className="w-5 h-5 text-blue-600" />} desc="Minimum uzunluk ve karmaşıklık.">
              <div className="grid md:grid-cols-4 gap-3">
                <div>
                  ...
                  <input type="number" min={6} className="w-full rounded-lg border-gray-300"
                    value={passwordPolicy.minLen} onChange={(e)=>setPasswordPolicy(p=>({ ...p, minLen: Number(e.target.value) }))}/>
                </div>
              ...
              <div className="mt-3 flex items-center justify-end">
                <button className="inline-flex items-center gap-2 px-3 py-2 rounded-lg bg-blue-600 text-white hover:bg-blue-700"><Save className="w-4 h-4" /> Kaydet</button>
              </div>
            </Section>

            <Section title="Oturum & 2FA" icon={<ShieldCheck className="w-5 h-5 text-blue-600" />}>
              <div className="grid md:grid-cols-2 gap-3">
                <div className="p-4 rounded-xl border">
                  <div className="flex items-center justify-between">
                    <div><div className="font-medium">2FA Zorunlu</div><div className="text-sm text-gray-500">Tüm kullanıcılar için iki adımlı doğrulama.</div></div>
                ...
                <div className="p-4 rounded-xl border">
                  <div className="flex items-center justify-between">
                    <div><div className="font-medium">Oturum Süresi</div><div className="text-sm text-gray-500">Otomatik çıkış için saat cinsinden.</div></div>
                    <div className="flex items-center gap-2">
              ...
              <Divider />
              <div className="flex items-center justify-between">
                <div><div className="font-medium">API Anahtarı</div><div className="text-sm text-gray-500">Hassas. Paylaşma.</div></div>
                <div className="flex items-center gap-2">
                  <code className="px-2 py-1 rounded bg-gray-50 border">{showApiKey ? apiKey : "•••••••••••••••"}</code>
                  <button className="p-2 rounded-lg border" onClick={()=>setShowApiKey(v=>!v)} title="Göster/Gizle">{showApiKey ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}</button>
                  <button className="p-2 rounded-lg border" onClick={rotateApiKey} title="Yenile"><RefreshCw className="w-4 h-4" /></button>
                </div>
              </div>
            </Section>
          </>
        ...
        {activeTab==="loglar" && (
          <Section title="Audit Log" icon={<Activity className="w-5 h-5 text-blue-600" />} desc="Önemli aksiyonlar.">
            <div className="overflow-x-auto">
              <table className="min-w-full text-sm">
                <thead><tr className="text-left text-gray-500"><th className="py-2">Zaman</th><th className="py-2">Kullanıcı</th><th className="py-2">Aksiyon</th><th className="py-2">Hedef</th></tr></thead>
                <tbody>
                  {audit.map(i=>(
